  ...
  return {
    ...
    server: (port = 8000) ->
      ...
      exec.run [
        ...
        "tmux send-keys -t #{session}:1 'vim -p app/demo.html app/stylesheets/demo.sass app/javascripts/demo.coffee; clear' C-m"
        "tmux new-window -t #{session}:2 -n 'server' 'gulp server'"
